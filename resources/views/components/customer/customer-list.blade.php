...
<script>
    ...
    //     const tableList = document.getElementById("dataList");
    ...
    async function getCustomersLists() {
        ...
        try{
            showLoader();
            let res= await axios.get(`/customers?page=${currentPage}&perPage=${perPage}&keyword=${keyword}`);
    ...
    function updateTable(data){
        ...
        if(!data.length){
            dataList.innerHTML='<tr><td colspan="4" class="text-center">No customers found.</td></tr>';
            return;
        }

        // Build the rows HTML string
        const rowsHtml= data.map(customer=>{
            return `
                <tr>
                    <td>${customer.name}</td>
                    <td>${customer.email}</td>
